...
private:
  ...
  struct Node {
    ...
    // creates a new node
    Node(Node* left, Node* right, value_type& pair);
  };
  ...
  // helper function for the copy constructor, works recursively
  void copyHelper(Node* a, Node* b);

  // height of a Node, nullptr is 0, root is 1, static, no access to 'this'
  // helper function to height(), used by printVertical
  static int getHeight(const Node *n);

  // helper function for the clear function, works recursively to delete nodes
  void clearNodes(Node* n);

  // helper function for the contains function, works recursively
  bool containsHelper(Node* n, const key_type& key, mapped_type& map) const;

  // helper function for the getAll function, works recursively
  void getAllHelper(Node* n, const key_type& k, vector<value_type>& v) const;

  // helper function for the inorder function, works recursively
  void inOrderHelper(Node* n, void visit(const value_type &item)) const;

  // helper function for the preorder function, works recursively
  void preOrderHelper(Node* n, void visit(const value_type &item)) const;

  // helper function for the postorder function, works recursively
  void postOrderHelper(Node* n, void visit(const value_type &item)) const;

  // helper function for the rebalance function, works recursively, puts all
  // the nodes in a vector through inorder traversal
  void nodesToVector(vector<value_type>& nodes, Node* n1);

  // helper function for the rebalance and nodesToVector function, works 
  // recursively, makes a tree from the given vector
  void buildTree(Node* temp, vector<value_type>& n, int begin, int end);

  // helper function for the operator== function, works recursively
  bool identicalHelper(Node* a, Node* b) const;

  // holds the total number of nodes in the BSTMap
  int totalNodes {0};
};
